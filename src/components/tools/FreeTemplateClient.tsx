...
import { ArrowLeft, Sparkles, Mail, ShieldCheck, Send, Gift, CheckCircle2, FileText } from 'lucide-react';

export default function FreeTemplateClient() {
      ...
      <div className="max-w-2xl mx-auto relative z-10 space-y-12">
        ...
        <div className="space-y-4">
          <motion.div
            ...
            className="w-20 h-20 bg-blue-600 rounded-[24px] flex items-center justify-center mx-auto mb-8 shadow-2xl shadow-blue-500/20"
          >
            <Gift className="w-10 h-10 text-white" />
          </motion.div>
          <h1 className="text-5xl md:text-7xl font-[1000] italic uppercase tracking-tighter leading-tight">
            FREE ATS <br /> <span className="text-blue-500">RESUME TEMPLATE</span>
          </h1>
          ...
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-[#0A0A0A] p-10 md:p-16 rounded-[60px] border border-white/5 shadow-2xl space-y-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              >
                {isLoading ? (
                  <>
                    <div className="w-6 h-6 border-2 border-white/20 border-t-white rounded-full animate-spin" /> Sending...
                  </>
                ) : (
                  <>
                    Get My Free Template <Send className="w-6 h-6" />
                  </>
                )}
              </button>
            </form>
