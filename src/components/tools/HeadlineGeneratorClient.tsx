      ...
      <div className="max-w-3xl mx-auto relative z-10 space-y-12">
        ...
        <div className="text-center space-y-4">
          <h1 className="text-4xl md:text-6xl font-[1000] italic uppercase tracking-tighter">
            Headline <span className="text-blue-500">Generator</span>
          </h1>
        ...
        <div className="bg-[#0A0A0A] p-10 rounded-[40px] border border-white/5 space-y-8">
          ...
          <button
            onClick={generate}
            className="w-full py-5 bg-blue-600 rounded-[24px] font-[1000] text-xl uppercase italic hover:bg-blue-500 transition-all shadow-xl"
          >
            Generate Headlines <Sparkles className="inline w-5 h-5 ml-2" />
          </button>
        </div>
        ...
        <div className="space-y-4">
          {headlines.map((h, i) => (
            <div
              key={i}
              className="p-6 bg-white/[0.02] border border-white/5 rounded-2xl flex items-center justify-between group hover:border-blue-500/30 transition-all"
            >
