      ...
      <div className="max-w-5xl mx-auto relative z-10 space-y-12">
        ...
        <div className="space-y-4">
          <h1 className="text-4xl md:text-6xl font-[1000] italic uppercase tracking-tighter">
            ATS Keyword <span className="text-blue-500">Scanner</span>
          </h1>
        ...
        <div className="grid md:grid-cols-2 gap-12">
          <div className="space-y-6">
            ...
            <button
              ...
              className="w-full py-6 bg-blue-600 rounded-[24px] font-[1000] text-xl uppercase italic hover:bg-blue-500 transition-all"
            >
              Scan Match
            </button>
          </div>
          ...
          <div className="bg-[#0A0A0A] border border-white/5 rounded-[40px] p-10 h-full relative overflow-hidden">
            {!results ? (
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  <h4 className="font-black uppercase tracking-widest text-xs text-green-500 flex items-center gap-2">
                    <Check className="w-4 h-4" /> Found ({results.matched.length})
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {results.matched.slice(0, 10).map((w: string) => (
                      <span key={w} className="px-3 py-1 bg-green-500/10 rounded-full text-[10px] font-bold text-green-400 uppercase">
                        {w}
                      </span>
                    ))}
                ...
                <div className="space-y-4">
                  <h4 className="font-black uppercase tracking-widest text-xs text-red-500 flex items-center gap-2">
                    <X className="w-4 h-4" /> Missing ({results.missing.length})
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {results.missing.slice(0, 10).map((w: string) => (
                      <span key={w} className="px-3 py-1 bg-red-500/10 rounded-full text-[10px] font-bold text-red-400 uppercase">
                        {w}
                      </span>
                    ))}
