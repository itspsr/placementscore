...
import { ArrowLeft, Calculator, Target, Briefcase, FileText, Award, Layers, Zap, Lock, ArrowRight } from 'lucide-react';
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="space-y-4 text-center md:text-left">
            ...
            <h1 className="text-5xl md:text-7xl font-[1000] italic uppercase tracking-tighter leading-tight">
              ATS SCORE <br /> <span className="text-blue-500">CALCULATOR</span>
            </h1>
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          <div className="bg-[#0A0A0A] p-10 md:p-14 rounded-[60px] border border-white/5 shadow-2xl space-y-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <CalcInput label="Experience (Years)" val={formData.exp} set={(v: number) => setFormData({ ...formData, exp: v })} icon={Briefcase} />
              <CalcInput label="Word Count" val={formData.words} set={(v: number) => setFormData({ ...formData, words: v })} icon={FileText} placeholder="e.g. 500" />
              <CalcInput label="Skills Mentioned" val={formData.skills} set={(v: number) => setFormData({ ...formData, skills: v })} icon={Target} />
              <CalcInput label="Projects Count" val={formData.projects} set={(v: number) => setFormData({ ...formData, projects: v })} icon={Layers} />
              <CalcInput label="Core Sections" val={formData.sections} set={(v: number) => setFormData({ ...formData, sections: v })} icon={Calculator} placeholder="Max 10" />
              <CalcInput label="Certifications" val={formData.certs} set={(v: number) => setFormData({ ...formData, certs: v })} icon={Award} />
            </div>
          ...
          <div className="space-y-8">
            <div className="bg-white/5 p-10 rounded-[50px] border border-white/5 space-y-8 relative overflow-hidden">
              <div className={`space-y-6 transition-all duration-1000 ${score === null ? 'blur-xl opacity-20 select-none' : ''}`}>
                ...
                <p className="text-lg text-white/50 leading-relaxed italic">
                  "Based on your input, your resume likely lacks the <strong>semantic depth</strong> required for Fortune 500 automated filters. Your projects-to-experience ratio is below standard."
                </p>
                <div className="h-px bg-white/10" />
                <div className="space-y-4">
                  <p className="text-xs font-black uppercase text-blue-500 tracking-widest flex items-center gap-2">
                    <Zap className="w-4 h-4 fill-current" /> High Impact Fixes:
                  </p>
            ...
            {score !== null && (
              <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="p-10 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[50px] shadow-2xl text-center space-y-6">
                <h3 className="text-2xl font-[1000] italic uppercase tracking-tighter leading-tight text-white">
                  Get Your Guaranteed <br /> 90+ Score Resume
                </h3>
