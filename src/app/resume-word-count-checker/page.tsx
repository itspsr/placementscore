    ...
    <main className="min-h-screen bg-[#050505] text-white p-6 pt-32 relative overflow-hidden">
      <div className="max-w-4xl mx-auto relative z-10 space-y-12">
        ...
        <div className="space-y-4">
           <h1 className="text-4xl md:text-6xl font-[1000] italic uppercase tracking-tighter">Resume Word <span className="text-blue-500">Counter</span></h1>
        ...
        <div className="grid md:grid-cols-2 gap-12">
           ...
           <div className="space-y-8">
              <div className={`p-10 rounded-[40px] border transition-all text-center space-y-4 ${status === 'good' ? 'bg-green-500/10 border-green-500/20' : status === 'bad' ? 'bg-red-500/10 border-red-500/20' : 'bg-white/5 border-white/10'}`}>
                 ...
                 <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest ${status === 'good' ? 'bg-green-500 text-white' : status === 'bad' ? 'bg-red-500 text-white' : 'bg-white/10 text-white/40'}`}>
                    {status === 'good' ? <CheckCircle2 className="w-4 h-4" /> : status === 'bad' ? <AlertCircle className="w-4 h-4" /> : <BarChart3 className="w-4 h-4" />}
              ...
              <div className="p-8 bg-white/[0.02] border border-white/5 rounded-[30px] space-y-4">
                 ...
                 <p className="text-sm text-white/40 leading-relaxed">Resumes under 400 words lack keyword density for matching algorithms. Resumes over 1000 words (for freshers) get penalized for relevance.</p>
              </div>
           </div>
        </div>
      </div>
